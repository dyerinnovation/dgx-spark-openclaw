...
- **Auth**: Anthropic setup token configured via CLI inside pod (`openclaw models auth paste-token --provider anthropic`), not via env var
...
- `.env` is the only file transferred via `scp` (it's gitignored)
